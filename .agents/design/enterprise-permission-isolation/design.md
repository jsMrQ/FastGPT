...
- [x] 1.8 手工回归：/account/team 页面五个 tab 全部无报错

### 阶段 2：资源授权闭环（隔离生效）
- [x] 2.1 C1/C2 app 协作者（含继承冲突与文件夹级联）
- [x] 2.2 C3/C4 dataset 协作者
- [x] 2.3 C5/C6 skill 协作者
- [x] 2.4 隔离验证：A 部门账号登录看不到 B 部门未共享资源；授权 read 后可见不可编辑
- [x] 2.5 全局搜索 `/proApi/` 核对剩余调用点，确认未实现项均有前端降级（不阻塞主流程）
